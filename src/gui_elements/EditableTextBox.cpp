...
EditableTextBox::EditableTextBox(std::string placeholder, std::string defaultText, int x, int y, int width, int height, bool renderCursor)
{
                    ...
                    if (event->key.keysym.mod & MODIFIER_KEY) {
                        if (selectionStart != selectionEnd) {
                            SDL_SetClipboardText(
                                text.substr(
                                    fmin(selectionStart, selectionEnd),
                                    abs(selectionStart - selectionEnd)
                                ).c_str());
                        }
                    }
                    break;
